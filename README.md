...
Currently there is no similar feature in place for Joomla or Wordpress sites, so the `admin.json` cookie file will have to be generated manually before running backstop (please note that the file needs to be generated only once, until the cookies reach the expiry date).

In order to tell `gulp` to skip the cookie creation phase, pass a `--skipCookies` argument to the task
